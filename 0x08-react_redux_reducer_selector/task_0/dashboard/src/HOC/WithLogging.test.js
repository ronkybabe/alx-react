...
describe("WithLogging tests", () => {
  it("should call console.log on mount and dismount", () => {
    ...
    const NewComponent = WithLogging({ WrappedComponent: TestComponent }); // Pass as an object
  ...
  it("should log out the right message on mount and on unmount", () => {
    ...
    expect(spy).toBeCalledWith("Component TestComponent is going to unmount"); // Corrected message
    spy.mockRestore();
  });
});
